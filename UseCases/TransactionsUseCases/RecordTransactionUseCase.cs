...
namespace UseCases
{
    public class RecordTransactionUseCase : IRecordTransactionUseCase
    {
        ...
        public void Execute(string cashierName, int productId, string productName, int soldQuantity)
        {
            var product = getProductByIdUseCase.Execute(productId);
            transactionRepository.Save(cashierName, productId, productName, product.Price.Value, product.Quantity.Value, soldQuantity);
        }
    }
}
